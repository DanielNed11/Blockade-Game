...
public class SelectSmallToBlockade extends Rule {
    @Override
    public boolean conditionRule(FactHandler facts) {
        return facts.factAvailable(FactValues.SELECT_SMALL_TO_BLOCKADE);
    }
    ...
    public boolean actionRule(FactHandler facts, GameSession game) {
        ...
        for (int row = 0; row < game.getBoard().getSize(); row++) {
            for (int col = 0; col < game.getBoard().getSize(); col++) {
                ...
                if (game.getAIColorOne() == currentTile.getTopPiece().getColor() ||
                        game.getAIColorTwo() == currentTile.getTopPiece().getColor()) {
                    if (game.canBlockade(currentTile)) {
